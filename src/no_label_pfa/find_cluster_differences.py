...
def find_cluster_differences(path_original_data,path_principal_features="principal_features0.txt",path_labels="dbscan_labels.csv",  clusters=[], number_output_functions=1,frac=1):
    ...
    pf_data = pd.read_csv(path_original_data, sep=',', header=None).to_numpy()[pfs].T
    clustering = pd.read_csv(path_labels, sep=',', header=None).to_numpy()

    data = pd.DataFrame(np.c_[clustering, pf_data].T)

    if len(clusters)>0:
        drop = [i for i in range(len(data.iloc[0])) if data.iloc[0][i] not in clusters]
        data.drop(columns=drop, inplace=True)

    config = ConfigParser()
    config.read("config.ini")
    min_n_datapoints_a_bin = config.getint("PFA PARAMETERS", "min_n_datapoints_a_bin")
    ...
    list_principal_features_global_indices = [[x] for x in left_features[number_output_functions:]]
    ...
    return principal_features_depending_on_system_state
